...
State_TileMapEditor::State_TileMapEditor(sf::RenderWindow &mWindow):
    ...
    , resourceManager()
    , tileEngineEditor()
    , guiManager(resourceManager, false)
...
void State_TileMapEditor::displayTextures()
{
    ...
    if(sf::Mouse::isButtonPressed(sf::Mouse::Right))
    {
        ...
        {
            tileEngineEditor.tileSelection(Data_Desktop::getInstance().getScaledMousePosition(window), window, coordinates);
        }

        tileEngineEditor.tile_hover(Data_Desktop::getInstance().getScaledMousePosition(window), window, selectionSize, coordinates);
    }
    else
    {
        if(sf::Mouse::isButtonPressed(sf::Mouse::Left))
        {
            if(selectedLayer != 3)
            {
                tileEngineEditor.replaceTile(Data_Desktop::getInstance().getScaledMousePosition(window), window, selectionSize, coordinates, selectedLayer);
            }
