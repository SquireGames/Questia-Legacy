    ...
    for(int x = 0; x < spawningInfo.size(); x++)
    {
            ...
            if(std::abs(coords.x - spawningInfo[x].coordinates.x) < 3000 )
            {
                if(std::abs(coords.y - spawningInfo[x].coordinates.y) < 2000 )
                {
                    ...
                    case 1:
                        ...
                        spawningInfo[x].ID = entityManager.createEntity(1, sf::Vector2f(spawningInfo[x].coordinates.x,spawningInfo[x].coordinates.y),entityManager);
                        //std::cout<<"SPAWNMAMAGER -- LIFE  "<< spawningInfo[x].ID  << std::endl;
                        break;
                    case 2:
                        spawningInfo[x].dead = false;
                        spawningInfo[x].cooldown = 500;
                        spawningInfo[x].ID = entityManager.createEntity(2, sf::Vector2f(spawningInfo[x].coordinates.x,spawningInfo[x].coordinates.y),entityManager);
                        break;
                    default:
                        //std::cout<<"ERROR";
                        break;
        ...
        else
        {
            if(spawningInfo[x].dead == true)
            {
                ...
                //std::cout<<spawningInfo[x].cooldown ;
            }
