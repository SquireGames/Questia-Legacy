...
void TileEngine_Editor::tile_hover(sf::Vector2f mousePosition, sf::RenderWindow &window, int selectionSize, sf::Vector2f coordinates)
{
...
void TileEngine_Editor::returnIDList(sf::RenderWindow &window)
{
    for(auto it = tileEngine.tileIndex.begin(); it != tileEngine.tileIndex.end(); it++)
    {
        ...
        {
            tileEngine.loadTiles(it->first , it2->first-1,it->first, it2->first, window);

            std::cout << "Tile Pairs: (" << it->first << ", " << it2->first << ")" << std::endl;
        }
    }
}

void TileEngine_Editor::replaceTile(sf::Vector2f mousePosition, sf::RenderWindow &window, int selectionSize,sf::Vector2f coordinates,int layer)
{
    mousePosition.x = (int)mousePosition.x/32;
    mousePosition.y = (int)mousePosition.y/32;
...
void TileEngine_Editor::tileSelection(sf::Vector2f mousePosition, sf::RenderWindow &window, sf::Vector2f coordinates)
{
    mousePosition.x = (int)mousePosition.x/32;
    mousePosition.y = (int)mousePosition.y/32;
...
void TileEngine_Editor::drawTileMap(sf::RenderWindow &window, sf::Vector2f coordinates, float scaler,int layer)
{
    ...
    for (int x=(coordinates.y-13 -scaler); x < (coordinates.y+10 + scaler); x++)
    {
        for (int y=(coordinates.x-20 - scaler); y < (coordinates.x+18 + scaler); y++)
        {
            ...
            {
                tileEngine.loadTiles(x, y, editTileMap[layer][x][y].x,editTileMap[layer][x][y].y, window);
            }
        }
    }
}
