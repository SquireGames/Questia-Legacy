...
TileEngine::TileEngine(ResourceManager& _resourceManager):
...
void TileEngine::local_loadMap(std::string mapName, int layer)
{
    mapHeight = 0;
    mapWidth = 0;
...
void TileEngine::loadMap(std::string mapName)//, SpawnManager& spawnManager)
{
    ...
    local_loadMap(mapSt, 0);
    local_loadMap(mapSt2, 1);
}
    ...
    for(int it = 0; it != layers; it++)
    {
        for (int x =(coordinates.y-13); x < (coordinates.y+10); x++)
        {
            for (int y =(coordinates.x-20); y < (coordinates.x+18); y++)
            {
                ...
                {
                    loadTiles(x, y, tileMap[it][x][y].x, tileMap[it][x][y].y, window);
                }
            }
        }
    }
...
void TileEngine::loadTiles(int x, int y, int index, int id, sf::RenderWindow &window)
{
...
bool TileEngine::tileIsUsed(int index, int id)
{
    if(index != 0 && id != 0)
    {
        return true;
